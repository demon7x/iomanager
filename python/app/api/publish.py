...
class Publish:
    ...
    def create_org_job(self):
        ...
        if self.master_input.retime_job:
            ...
            cmd = ['rez-env', 'nuke-13', '--', 'nuke', '-ix', self.nuke_retime_script]
            if not self.scan_colorspace.find("ACES") == -1:
                cmd = ['rez-env', 'nuke-13', 'ocio_config', '--', 'nuke', '-ix', self.nuke_retime_script]
            if not self.scan_colorspace.find("Alexa") == -1:
                cmd = ['rez-env', 'nuke-13', 'alexa_config', '--', 'nuke', '-ix', self.nuke_retime_script]
            if not self.scan_colorspace.find("legacy") == -1:
                cmd = ['rez-env', 'nuke-13', 'legacy_config', '--', 'nuke', '-ix', self.nuke_retime_script]
            if not self.scan_colorspace.find("Sony") == -1:
                cmd = ['rez-env', 'nuke-13', 'sony_config', '--', 'nuke', '-ix', self.nuke_retime_script]
            if not self.scan_colorspace.find("Arri") == -1:
                cmd = ['rez-env', 'nuke-13', 'alexa4_config', '--', 'nuke', '-ix', self.nuke_retime_script]
            ...
            if not self.scan_colorspace.find( 'Output' ) == -1:
                cmd = ['rez-env', 'nuke-13', 'ocio_config', '--', 'nuke', '-ix', self.nuke_mov_script]
            if not self.scan_colorspace.find("Alexa") == -1:
                cmd = ['rez-env', 'nuke-13', 'alexa_config', '--', 'nuke', '-ix', self.nuke_mov_script]
            if not self.scan_colorspace.find("legacy") == -1:
                cmd = ['rez-env', 'nuke-13', 'legacy_config', '--', 'nuke', '-ix', self.nuke_mov_script]
            if not self.scan_colorspace.find("Sony") == -1:
                cmd = ['rez-env', 'nuke-13', 'sony_config', '--', 'nuke', '-ix', self.nuke_mov_script]
            if not self.scan_colorspace.find("Arri") == -1:
                cmd = ['rez-env', 'nuke-13', 'alexa4_config', '--', 'nuke', '-ix', self.nuke_mov_script]
                if self.project['name'] == "jung" or self.project['name'] == 'RND':
...
#            print('\n')
    ...
    def _create_temp_jpg_job(self, temp_path, temp_name):
        ...
        if not self.scan_colorspace.find("ACES") == -1:
            cmd = ['rez-env', 'nuke-13', 'ocio_config', '--', 'nuke', '-ix', tmp_org_jpg_script]
        if not self.scan_colorspace.find("Alexa") == -1:
            cmd = ['rez-env', 'nuke-13', 'alexa_config', '--', 'nuke', '-ix', tmp_org_jpg_script]
        if not self.scan_colorspace.find("legacy") == -1:
            cmd = ['rez-env', 'nuke-13', 'legacy_config', '--', 'nuke', '-ix', tmp_org_jpg_script]
        if not self.scan_colorspace.find("Sony") == -1:
            cmd = ['rez-env', 'nuke-13', 'sony_config', '--', 'nuke', '-ix', tmp_org_jpg_script]
        if not self.scan_colorspace.find("Arri") == -1:
            cmd = ['rez-env', 'nuke-13', 'alexa4_config', '--', 'nuke', '-ix', tmp_org_jpg_script]
    ...
    def _create_copy_script(self):
        ...
        tmp_copy_script_file = os.path.join(AppConfig.get_project_path(), 'seq',
                                            self.seq_name,
    ...
    def create_version(self, switch=False):
        ...
        mov_path = os.path.join(AppConfig.get_project_path(), 'seq',
                                self.seq_name,
                                self.shot_name, "plate",
                                plate_name + ".mov")
        ...
        if self.master_input.ext in ["mov","mxf"] and self.seq_type != 'lib':
            ...
            read_path = os.path.join(AppConfig.get_project_path(), 'seq',
                                     self.seq_name,
                                     self.shot_name, "plate",
        ...
        if self.seq_type == "editor" and self.master_input.clip_tag:
            desc['sg_retime_info'] = self.master_input.clip_tag
    ...
    def create_jpg_job(self, switch=False):
        ...
        cmd = ['rez-env', 'nuke-13', '--', 'nuke', '-ix', self.nuke_script]
        if self._opt_dpx == True:
            ...
            if not self.scan_colorspace.find("Arri") == -1:
                cmd = ['rez-env', 'nuke-13', 'alexa4_config', '--', 'nuke', '-ix', self.nuke_script]
    ...
    def convert_mp4_job(self, switch=False):
        ...
        if self.seq_type != 'lib':
            ...
            mov_path = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
                                    file_name + ".mov")
            mp4_path = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
                                    file_name + ".mp4")
    ...
    def create_nuke_temp_script(self, read_path):
        ...
        tmp_org_jpg_file = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
    ...
    def create_nuke_retime_script(self):
        if self._opt_non_retime == True and os.path.exists(self.plate_path):
            return None
        # AppInstance를 통해 컨텍스트 및 Shotgun API 가져오기
        from python.app import get_context, get_shotgun
        ...
        tmp_nuke_script_file = os.path.join(AppConfig.get_project_path(), 'seq',
                                            self.seq_name,
                                            self.shot_name, "plate",
    ...
    def create_mov_nuke_script(self):
        ...
        colorspace = self.scan_colorspace.replace("ACES-", "")
        org_path = os.path.join(AppConfig.get_project_path(), 'seq',
                                self.seq_name,
                                self.shot_name, "plate",
                                self.plate_file_name + "_" + colorspace + ".mov")
        mov_path = os.path.join(AppConfig.get_project_path(), 'seq',
        ...
        #     self.use_natron = True
            ...
            if self.scan_colorspace == 'AlexaV3LogC':
                self.jpg4mov_alexaV3logC_py = os.path.join(AppConfig.get_project_path(), 'seq',
                                                    self.seq_name,
                                                    self.shot_name, "plate",
                ...
                with open( self.jpg4mov_alexaV3logC_py, 'w' ) as f:
                    f.write( jpg4mov_nk )
                nk += 'os.system( "rez-env nuke-13 alexa_config -- nuke -ix {}" )\n'.format( self.jpg4mov_alexaV3logC_py )
                # nk += 'os.remove( "{}" )\n'.format( self.jpg4mov_alexaV3logC_py )
...
#            nk += 'write.getOutputFormat().set(0,0,{0},{1})\n'.format(int(width), int(height))
    ...
    def create_nuke_script(self):
        ...
        if self._opt_dpx == False:
            ...
            # org/src type flip/flop Mirror node
            if self.seq_type in ['org', 'src'] and self.master_input.clip_tag:
                clip_tag = self.master_input.clip_tag.strip()
                if clip_tag in ['flop', 'flip']:
                    nk += 'mirror = nuke.nodes.Mirror(inputs=[%s])\n' % tg
                    mirror_param = 'Horizontal' if clip_tag == 'flop' else 'Vertical'
                    nk += 'mirror["%s"].setValue(True)\n' % mirror_param
                    tg = 'mirror'
        ...
        # -> 프로젝트 종료 후에도 aces_config 사용하면 natron 대신 nuke 사용할 수도 있음
            ...
            self.tmp_dpx_to_jpg_file = os.path.join(AppConfig.get_project_path(), 'seq',
                                                   self.seq_name,
                                                   self.shot_name, "plate",
    ...
    def create_sg_script(self, switch=False):
        if self._opt_non_retime == True and switch == False and os.path.exists(self.plate_path):
            return None
        if self.seq_type != 'lib':
            tmp_sg_script_file = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
        ...
        if self.seq_type != 'lib':
            mov_path01 = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
                                      self.plate_file_name + ".mov")
            mp4_path01 = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
    ...
    @property
    def webm_path(self):
        temp = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name, self.shot_name, "plate",
    ...
    @property
    def plate_path(self):
        temp = os.path.join(AppConfig.get_project_path(), 'seq', self.seq_name,
    ...
    def publish_to_shotgun(self):
        ...
        if self.master_input.ext in ["mov","mxf"]:
            if self._opt_dpx == False:
                published_file = os.path.join(AppConfig.get_project_path(), 'seq',
                                              self.seq_name,
                                              self.shot_name, "plate",
