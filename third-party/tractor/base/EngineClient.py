...
class EngineClient(object):
    ...
    SPOOL = "spool"
    DB = "db"
    VALID_PARAMETERS = ("hostname", "port", "user", "password", "debug", "newSession", "sessionFilename")
    ...
    def __init__(self, hostname=None, port=None, user=None, password=None, sessionFilename=None, debug=False):
    ...
    def setParam(self, **kw):
        ...
        for key, value in list(kw.items()):
            if key not in self.VALID_PARAMETERS: 
                raise InvalidParamError("%s is not a valid parameter.  Must be in %s." %
    ...
    def dprintUrl(self, url):
        ...
        if self.debug:
            trutil.log("[%s:%s] http://%s:%s/Tractor/%s" % (self.hostname, self.port, self.hostname, self.port, url))
    ...
    def canReuseSession(self):
        ...
        # test session id
        try:
            self._transaction(self.CONTROL, {"q": "status", "tsid": tsid}, skipLogin=True)
    ...
    def open(self):
        ...
        has been set.
        """
        self.dprint("open(), self.newSession=%s, self.sessionFilename=%s" %
        ...
        if not self.password:
            ...
            if self.conn.PasswordRequired():
                self.dprint("Password required for %s@%s:%d " % (self.user, self.hostname, self.port))
                raise PasswordRequired("Password required for %s@%s:%d " % (self.user, self.hostname, self.port))
        ...
        try:
            ...
            if re.findall("login as '.*' failed", err):
                msg = "Unable to log in as user %s on engine %s:%s." % (self.user, self.hostname, self.port)
            else:
        ...
        if self.tsid == None:
            msg ="Error logging in as user %s on engine %s:%s: %s" % \
                  (self.user, self.hostname, self.port, str(err))
            self.dprint(msg)
    ...
    def writeSessionFile(self):
        ...
        if not os.path.exists(directory):
            try:
                ...
                msg = "problem creating session directory '%s': %s" % (str(directory), str(err))
                self.dprint(msg)
                raise CreateSessionDirectoryError(msg)

        try:
            with os.fdopen(os.open(self.sessionFilename, os.O_WRONLY | os.O_CREAT, 0o600), "w") as f:
                    f.write('{"tsid": "%s"}\n' % self.tsid);
        except (IOError, OSError) as err:
            msg = "problem writing session file '%s': %s" % (str(self.sessionFilename), str(err))
            self.dprint(msg)
    ...
    def constructURL(self, queryType, keyValuePairs):
        """Build a URL."""
        parts = []
        for key, value in list(keyValuePairs.items()):
    ...
    def _transaction(self, urltype, attrs, payload=None, translation="JSON", headers={}, skipLogin=False):
        ...
        if rcode:
            print(rcode)
            try:
                datadict = ast.literal_eval(str(data.a))
    ...
    def getTaskCommands(self, jid, tid):
        ...
        for cmd in cmds:
            cmdObj = DictObj(**cmd)
            line = "{cid} {state} {service} {tags} {type} {t0} {t1} {argv}"\
                   .format(cid=cmdObj.cid, state=cmdObj.state, service=cmdObj.service,
                           tags=cmdObj.tags, type=cmdObj.type,
                           t0=cmdObj.t0, t1=cmdObj.t1, argv=cmdObj.argv)
            lines.append(line)
    ...
    def spool(self, jobData, hostname=None, filename=None, owner=None, format=None, skipLogin=False, block=False):
        ...
        attrs = {"spvers": self.SPOOL_VERSION, "hnm": hostname, "jobOwner": owner, "jobAuthor": author, "jobFile": filename,
                 "cwd": cwd}
        if block:
