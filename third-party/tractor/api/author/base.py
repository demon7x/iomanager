...
class Assign(Element):
    ...
    def __init__(self, varname, value):
        if not isinstance(varname, str):
            raise TypeError("Assign.__init__(): varname %s is not a "
                            "string" % str(varname))
        if not isinstance(value, str):
            raise TypeError("Assign.__init__(): value %s is not a "
                            "string" % str(value))
...
class DirMap(Element):
    ...
    def __init__(self, src, dst, zone):
        if not isinstance(src, str):
            raise TypeError("DirMap.__init__(): src %s is not a "
                            "string" % str(varname))
        if not isinstance(dst, str):
            raise TypeError("DirMap.__init__(): dst %s is not a "
                            "string" % str(value))
        if not isinstance(zone, str):
            raise TypeError("DirMap.__init__(): zone %s is not a "
                            "string" % str(value))
        self.src = src
        self.dst = dst
        self.zone = zone
...
class SubtaskMixin(object):
    ...
    def spool(self, block=False, owner=None, spoolfile=None, spoolhost=None,
        ...
        try:
            result = ModuleEngineClient.spool(
                ...
                filename=spoolfile, hostname=spoolhost)
